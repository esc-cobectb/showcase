...
class Cyclingnews extends \Openurbantech\Stream\Manager
{
 ...
 //    }
	...
	public function processItems($content)
	{
		...
		$elements = $xpath->query("//a[@class='article-link']");
		foreach ($elements as $index => $node) {
			$url = $node->getAttribute("href");
			if(!empty($url)){
				$items[$index]['url'] = $url;
			}
			$title =  trim($node->getAttribute("aria-label"));
			if(!empty($title) && !empty($items[$index])){
				$items[$index]['title'] = $title;
			}
		}
		$elements = $xpath->query("//a[@class='article-link']//time");
		foreach ($elements as $index => $node) {
